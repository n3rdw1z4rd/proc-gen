...
export class Noise {
    private static _instance: Noise;
    ...
    fractalNoise2d(
        ...
        const { octaves, frequency, persistence, amplitude } = params;
        let height = 0.0;
    ...
    public static get instance(): Noise {
        if (!Noise._instance) {
            Noise._instance = new Noise();
        }

        return Noise._instance;
    }
}

// export function noise_T_<T extends number[]>(...components: [...T, NoiseParams]) { }

export function noise(x: number, y: number, z?: number | NoiseParams, params?: NoiseParams): number {
    if (typeof z !== 'number') {
        params = z;
        z = 0;
    }
    ...
    if (params) {
        if (!z)
            result = Noise.instance.fractalNoise2d(x, y, params);
        else
            result = Noise.instance.fractalNoise3d(x, y, z, params);
    } else {
        if (!z)
            result = Noise.instance.noise2d(x, y);
        else
            result = Noise.instance.noise3d(x, y, z);
    }
